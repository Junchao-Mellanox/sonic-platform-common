        ...
        for page in pages:
            if page == 0:
                if self.is_active_cable():
                    ...
                    lines.append(self._dump_eeprom(0, 256, indent))
                else:
                    lines.append(f'{indent}A0h dump')
                    lines.append(self._dump_eeprom(0, 128, indent))
            elif page == 1:
                lines.append(f'\n{indent}A2h dump (lower 128 bytes)')
                lines.append(self._dump_eeprom(256, 128, indent))
            #elif page == 2:
            #    lines.append(f'\n{indent}A2h dump (upper 128 bytes)')
            #    lines.append(self._dump_eeprom(256 + 128, 128, indent))
        return '\n'.join(lines)
